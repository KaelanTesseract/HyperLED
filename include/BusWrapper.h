...
// HUB75 scan-matrix panel, driven via the esp-hub75 library's DMA engine (PARLIO
// on ESP32-C6, GDMA on S3, I2S on the classic ESP32). Pixels are addressed as a
// flat index like every other IBus, but internally mapped back to (x, y) since
// the HUB75 driver takes x/y coordinates rather than a linear buffer offset.
...
public:
    BusHub75(uint16_t width, uint16_t height, Hub75ShiftDriver shiftDriver) : _width(width), _height(height) {
        ...
        // panels/bit depths (double buffering would need twice as much). Brightness stays
        // at max - HyperLED already scales r/g/b by the segment's own brightness/ABL before
        // calling SetPixelColor, so the driver's own brightness knob would otherwise double-dim.
